...
$sql = "SELECT * FROM bd_tipo_prod WHERE nome_tipo_prod = '{$_POST[nome_tipo_prod]}'";
$res = mysql_query($sql) or die("Erro seleção");

if (mysql_num_rows($res) and !isset($_POST[id_bd_tipo_prod])){
    ...
    elseif(isset($_POST[id_bd_marcas])){
        $sql =
        "UPDATE bd_tipo_prod SET nome_tipo_prod='{$_POST[nome_tipo_prod]}'";
            
    //volta para a seleção e alteração
    $lugar=('lista_lote_alt.php');
}
    else {
    $sql = "INSERT into bd_tipo_prod (nome_tipo_prod)
                       VALUES ('{$_POST[nome_tipo_prod]}')";
...
?>
    <tr>
        <td width="490" colspan="2" align="left" bgcolor="#FFFF00" class="A" scope="col">
            OK, TIPO PRODUTO cadastrado com sucesso!
            <input type='button' value='Novo'
               onclick='document.location.href="cad_bd_tipo_prod.php"'>
