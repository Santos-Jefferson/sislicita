...
$vunitcusto_verba = ($_POST[vunitcusto]-$_POST[verba_mkt]);
//print_r ($_REQUEST);
//die;
...
$sql = "SELECT * FROM itens_ne WHERE item = {$_POST[item]} and id_lote = {$_POST[id_lote]}";
$res = mysql_query($sql) or die("Erro seleção grava_itens_ne");

if (mysql_num_rows($res) and !isset($_POST[id_itens_ne])){
    
    require_once "validacao_cad_itens.php";
    die;
   
 }
    elseif(isset($_POST[id_itens_ne])){
        $sql =
        "UPDATE itens_ne SET item='{$_POST[item]}',qtde='{$_POST[qtde]}', nome_item='{$_POST[nome_item]}',marca_item='{$_POST[marca_item]}',modelo_item='{$_POST[modelo_item]}',ro_item='{$_POST[ro_item]}',
            vunitcusto='$vunitcusto_verba',vtotcusto='{$_POST[vtotcusto]}',
            ...
            vitem='{$_POST[vitem]}',vminitem='{$_POST[vminitem]}',porc_lucro='{$_POST[porc_lucro]}'
                WHERE id_itens_ne='{$_POST[id_itens_ne]}'
            ";
    ...
    $sql = "INSERT into itens_ne (item,qtde,nome_item,marca_item,modelo_item,ro_item,vunitcusto,vtotcusto,forn,vfrete,imposto,custo_aprox,lucro_liq,vofertado,vminofertar,vitem,vminitem,porc_lucro,id_lote)
                       VALUES ('{$_POST[item]}','{$_POST[qtde]}','{$_POST[nome_item]}','{$_POST[marca_item]}','{$_POST[modelo_item]}','{$_POST[ro_item]}',
                               '$vunitcusto_verba','{$_POST[vtotcusto]}','{$_POST[forn]}',
