...
if(empty($_REQUEST[order])) $_REQUEST[order] = 'sub_itens';
    
$sql = "SELECT * FROM subitens WHERE id_item = {$_REQUEST[id_item]} order by {$_REQUEST[order]}";
...
<table align="center" class="A" width="1000" border="1" cellpadding="3" cellspacing="0">
    ...
    <tr>
        <td class="forms" colspan="8" bgcolor="LemonChiffon" align="center"><strong>LISTAGEM SUB-ITENS DO ITEM N° <?php echo $_GET[item] ?> e LOTE/GRUPO N° <?php echo $_GET[lote] ?></td>
    </tr>
...
while($linha){
    ...
    $vtc += $linha[vtotcusto_si];    //incrementa a $v o campo valor
...
<td>&nbsp;<a href="altera_sub_itens.php?alterar=<?php echo $linha[id_subitem]; ?>&lote=<?php echo $_GET[lote]; ?>&item=<?php echo $_GET[item]; ?>&id_item=<?php echo $_GET[id_item]; ?>&orgao=<?php echo $_GET[orgao]; ?>&data=<?php echo $_GET[data]; ?>&hora=<?php echo $_GET[hora]; ?>&licitante=<?php echo $_GET[licitante]; ?>&id_lote=<?php echo $_GET[id_lote]; ?>"><?php echo $linha[qtde_si] ?></a>
    ...
    <td>&nbsp;<?php echo $linha[sub_itens] ?></td>
    ...
    <td>&nbsp;<?php 
                $number = "$linha[vtotcusto_si]";
                echo "R$" .number_format($number,2, ',','.');
                ?>
    </td>
    <td>&nbsp;<?php echo $linha[obs_si] ?></td>
...
?>
  ...
  <tr>
    ...
    <tr>
        <td colspan="8">Total de sub-itens: <?php echo $c ?></td>
