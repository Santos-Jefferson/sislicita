...
if (mysql_num_rows($res) and !isset($_POST[id_itens])){
    ...
    elseif(isset($_POST[id_itens])){
        $sql =
        "UPDATE itens SET item='{$_POST[item]}',qtde='{$_POST[qtde]}', nome_item='{$_POST[nome_item]}',marca_item='{$_POST[marca_item]}',modelo_item='{$_POST[modelo_item]}',ro_item='{$_POST[ro_item]}',
    ...
    $sql = "INSERT into itens (item,qtde,nome_item,marca_item,modelo_item,ro_item,vunitcusto,vtotcusto,forn,vfrete,imposto,custo_aprox,lucro_liq,vofertado,vminofertar,vitem,vminitem,porc_lucro,id_lote)
