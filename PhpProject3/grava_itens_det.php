...
echo $sql = "SELECT * FROM subitens where id_item = {$_POST[id_item]}"; //{$_POST[qtde_si]}"; //WHERE sub_itens = {$_POST[id_item]}";
...
//if (mysql_num_rows($res) and !isset($_POST[id_subitem])){
 ...
 //}
    if(isset($_POST[id_subitem])){
        echo $sql =
        "UPDATE subitens SET qtde_si='{$_POST[qtde_si]}', marca_si='{$_POST[marca_si]}',
                               modelo_si='{$_POST[modelo_si]}',sub_itens='{$_POST[sub_itens]}',
                               vunitcusto_si='{$_POST[vunitcusto_si]}',vtotcusto_si='{$_POST[vtotcusto_si]}',
                               obs_si='{$_POST[obs_si]}',ro_subitem='{$_POST[ro_subitem]}',id_item='{$_POST[id_item]}'
    ...
    $sql = "INSERT into subitens (qtde_si,marca_si,modelo_si,sub_itens,vunitcusto_si,vtotcusto_si,obs_si,ro_subitem,id_item)
                       VALUES ('{$_POST[qtde_si]}','{$_POST[marca_si]}',
                               '{$_POST[modelo_si]}','{$_POST[sub_itens]}',
                               '{$_POST[vunitcusto_si]}','{$_POST[vtotcusto_si]}',
                               '{$_POST[obs_si]}','{$_POST[ro_subitem]}','{$_POST[id_item]}'
                               )";
    $lugar="cad_itens_det.php?id_item=$_POST[id_item]";
    }
